...
public class Genetics {
	public static double fitnessFunction(ArrayList<Integer> state) ////// evaluate number of pairwise threats
	{
		double eval = 0;
                double counter = 0;
                double queenNumber = 0;
                double queenRate = 0;
                int[][] board = new int [state.size()][state.size()];
                ...
                queenRate = queenNumber/state.size();
               // System.out.println("queenRate :"+queenRate+"queen : "+queenNumber);
                for (int i = 0; i < state.size(); i++) {
                        ...
                        if(state.get(i)>=0)
                            {  
                                ...
                                {
                                    board[j][i] = 1;
                                     board[state.get(i)][j] = 1;
                                     if(j<=i)
                                            if(state.get(i)>j && i>j)
                                            board[state.get(i)-j-1][i-j-1] = 1;
                                            if(state.get(i)>j && i+j<state.size()-1)
                                            board[state.get(i)-j-1][i+j+1] = 1;
                                           if(state.get(i)+j<state.size()-1 && i>j)
                                            board[state.get(i)+j+1][i-j-1] = 1;
                                            if(state.get(i)+j<state.size()-1 && i+j<state.size()-1)
                                           board[state.get(i)+j+1][i+j+1] = 1;
                ...
                for(int j=0;j<state.size();j++)
                    if(board[i][j] == 1)
                        counter++;
                    System.out.println("queen number= "+queenNumber);
                    System.out.println("board number= "+counter);
                    eval = (10*(counter/(state.size()*state.size()))+1/(queenRate))/11;
                    if((counter/(state.size()*state.size()))==1)
...
*/
	...
	public static ArrayList<ArrayList<Integer>> randomPopulation(int chromosomeSize, int populationSize) {
		...
		ArrayList<ArrayList<Integer>> randomPopulation = new ArrayList<ArrayList<Integer>>();
		for (int i = 0; i < chromosomeSize; i++)
			sample.add(randomNumber.nextInt(chromosomeSize+chromosomeSize)-chromosomeSize);
	...
	public static ArrayList<Integer> mutation(ArrayList<Integer> chromosome) {
		Random randomNumber = new Random();
		int mutationPoint = randomNumber.nextInt(chromosome.size());
		int mutationValue = randomNumber.nextInt(chromosome.size());
		chromosome.set(mutationPoint, mutationValue);
		return chromosome;
	}

	public static ArrayList <ArrayList<Integer>> crossover(ArrayList<Integer> parent1, ArrayList<Integer> parent2) {
	...
	public static void crossover(ArrayList<ArrayList<Integer>> population, double propability) {
		...
		for (int i = 0; i < populationSize; i++) {
			...
			if (chance < propability) {
				population.addAll(crossover(population.get(i), population.get(i + 1)));
			}
		}
	}
			...
			for (int j=0;j<populations.size();j++) 
			{
				for (int i=0;i<populations.get(j).size();i++) 
				{
					...
					if (chance < probability) {
						...
						populations.get((destination+j) % populations.size()).add(populations.get(j).get(i));
	...
	public static void printPopualation(ArrayList<ArrayList<Integer>> population)
	{
		for(ArrayList<Integer> chromosome: population)
		{
			System.out.println(chromosome+" fitness= "+ fitnessFunction(chromosome));
		}	
	}
	public static double average(ArrayList<ArrayList<Integer>> population)
	{
		...
		for(ArrayList<Integer> chromosome: population)
		{
			sum=fitnessFunction(chromosome)+sum;
		}	
	...
	public static ArrayList<ArrayList<Integer>> Genetic(int chromosomeSize,int populationSize,int generationCount) throws IOException
	{
		...
		population=randomPopulation(chromosomeSize, 10*populationSize);
